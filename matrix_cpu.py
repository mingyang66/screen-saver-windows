...
LOG_PATH = APP_DIR / "matrix_cpu.log"
...
def get_logger():
    ...
    logger = logging.getLogger("matrix_cpu")
...
def format_rate(bytes_per_second):
    """Format a byte rate into a compact fixed-width string."""
    value = float(max(0.0, bytes_per_second))
    for unit in ("B", "K", "M"):
        if value < 1000.0:
            return f"{value:5.1f}{unit}/S"
        value /= 1000.0
    return f"{value:5.1f}G/S"


class GpuProbe:
    """Best-effort NVIDIA GPU probe; disabled when NVML is unavailable."""
    ...
    def __init__(self):
        ...
        try:
            ...
            LOGGER.info("GPU metrics unavailable: %s", exc)

    @property
    def available(self):
        return self.pynvml is not None

    def sample(self):
        if not self.available:
            return None
        try:
            pynvml, handle = self.pynvml, self.handle
            ...
            return {
                ...
                "gpu_mem_used": memory.used / max(1, memory.total),
                "gpu_name": self.name,
            }
        except Exception as exc:
            LOGGER.info("GPU sample failed: %s", exc)
            return None


class CpuTempProbe:
    """Read ACPI thermal zones through WMI; needs pywin32 and may need admin."""
    ...
    def __init__(self):
        ...
        try:
            ...
            self.scope = win32com.client.GetObject("winmgmts:root\\wmi")
        except Exception as exc:
            LOGGER.info("CPU temperature unavailable: %s", exc)
    ...
    def sample(self):
        if not self.available:
            return None
        try:
            zones = self.scope.InstancesOf("MSAcpi_ThermalZoneTemperature")
            temps = [(zone.CurrentTemperature - 2732) / 10.0 for zone in zones]
            return max(temps) if temps else None
        except Exception as exc:
            LOGGER.info("CPU temperature sample failed: %s", exc)
            return None


class MetricsCollector(threading.Thread):
    """Background sampler feeding both the dashboard panel and the rain feed."""

    def __init__(self):
        super().__init__(daemon=True, name="matrix-metrics")
    ...
    def snapshot(self):
        with self.lock:
            return dict(self.data), list(self.cpu_history), list(self.gpu_history)

    def run(self):
        gpu_probe = GpuProbe()
        temp_probe = CpuTempProbe()
        if psutil is not None:
            psutil.cpu_percent(None)
            self._last_net = psutil.net_io_counters()
            self._last_net_time = time.monotonic()
        else:
            LOGGER.warning("psutil is not installed; hardware metrics disabled")
        sample_index = 0
        next_tick = time.monotonic()
        while True:
            self.collect(gpu_probe, temp_probe, sample_index)
            sample_index += 1
            next_tick += METRICS_INTERVAL
            time.sleep(max(0.0, next_tick - time.monotonic()))

    def collect(self, gpu_probe, temp_probe, sample_index):
        with self.lock:
            data = dict(self.data)
        if psutil is not None:
            data["cpu_percent"] = psutil.cpu_percent(None)
            data["mem_percent"] = psutil.virtual_memory().percent
            net = psutil.net_io_counters()
            now = time.monotonic()
            if self._last_net is not None:
                interval = max(0.1, now - self._last_net_time)
                data["net_up"] = max(0.0, net.bytes_sent - self._last_net.bytes_sent) / interval
                data["net_down"] = max(0.0, net.bytes_recv - self._last_net.bytes_recv) / interval
            self._last_net = net
            self._last_net_time = now
        gpu = gpu_probe.sample()
        if gpu is not None:
            data.update(gpu)
        if temp_probe.available and sample_index % TEMP_REFRESH_SAMPLES == 0:
            temperature = temp_probe.sample()
            if temperature is not None:
                data["zone_temp"] = temperature
        data["stamp"] = time.strftime("%H:%M:%S")
        with self.lock:
            self.data = data
            self.cpu_history.append(min(100.0, data["cpu_percent"]))
            if data["gpu_percent"] is not None:
                self.gpu_history.append(min(100.0, data["gpu_percent"]))
...
def get_metrics():
    """Start the shared metrics collector once and reuse it for every window."""
    global _metrics
    if _metrics is None:
...
class MatrixDashboard(QWidget):
    ...
    def __init__(self, config, preview=False, screen=None, preview_handle=None):
        ...
        self.metrics_data, self.cpu_history, self.gpu_history = self.metrics.snapshot()
        ...
        self.blue = QColor("#00AAFF")
        self.amber = QColor("#FFB000")
        self.white = QColor("#FFFFFF")
        self.text_shadow = QColor(0, 0, 0)
    ...
    def draw_panel_frame(self, painter, panel, scale):
        """Draw the rounded glow border without an opaque background."""
        radius = 14 * scale
        painter.setBrush(Qt.NoBrush)
        for width, alpha in ((7, 22), (4, 46)):
            painter.setPen(QPen(QColor(0, 255, 0, alpha), max(1.0, width * scale)))
            painter.drawRoundedRect(panel, radius, radius)
        painter.setPen(QPen(self.green, max(1, round(1 * scale))))
        painter.drawRoundedRect(panel, radius, radius)

    def draw_text(self, painter, rect, flags, text):
        """Draw panel text with a dark shadow so it reads over the rain."""
        offset = 2 * self.layout_scale()
        pen = painter.pen()
    ...
    def draw_dashboard(self, painter):
        ...
        panel_height = min(self.height() * 0.86, max(480 * scale, (386 + len(self.config.todos) * 30) * scale))
        panel = QRectF((self.width() - panel_width) / 2, (self.height() - panel_height) / 2, panel_width, panel_height)
        margin = max(16, 32 * scale)
        self.draw_panel_frame(painter, panel, scale)
        painter.setPen(self.white)

        content_left = panel.left() + margin
        content_width = panel.width() - margin * 2

        title_rect = QRectF(content_left, panel.top() + 10 * scale, content_width, 42 * scale)
        painter.setFont(self.title_font)
        self.draw_text(painter, title_rect, Qt.AlignCenter, self.config.title)
        painter.setFont(self.time_font)
        self.draw_text(painter, QRectF(content_left, panel.top() + 58 * scale, content_width, 28 * scale), Qt.AlignCenter, self.info_time)

        # Keep the weekday on its own line so the date and time remain readable.
        painter.setFont(self.progress_font)
        weekday_rect = QRectF(content_left, panel.top() + 86 * scale, content_width, 22 * scale)
        self.draw_text(painter, weekday_rect, Qt.AlignCenter, self.info_weekday)
        ...
        label_rect = QRectF(content_left, progress_y, label_width, 24 * scale)
        ...
        self.draw_text(painter, label_rect, Qt.AlignVCenter | Qt.AlignLeft, "DAY PROGRESS")
        ...
        self.draw_text(painter, percent_rect, Qt.AlignVCenter | Qt.AlignRight, f"{self.info_progress * 100:.2f}%")

        divider_y = panel.top() + 160 * scale
        painter.setPen(QPen(self.green, 1, Qt.DashLine))
        painter.drawLine(content_left, divider_y, panel.right() - margin, divider_y)
        painter.setPen(self.green)
        self.draw_metrics(painter, panel, scale, content_left, content_width)

        painter.setPen(QPen(self.green, 1, Qt.DashLine))
        painter.drawLine(content_left, panel.top() + 330 * scale, panel.right() - margin, panel.top() + 330 * scale)
        painter.setPen(self.white)
        painter.setFont(self.todo_font)
        metrics = painter.fontMetrics()
        for index, item in enumerate(self.config.todos):
            text = metrics.elidedText(item, Qt.ElideRight, int(content_width))
            rect = QRectF(content_left, panel.top() + 338 * scale + index * 30 * scale, content_width, 26 * scale)
            self.draw_text(painter, rect, Qt.AlignVCenter | Qt.AlignLeft, text)
        painter.setFont(self.footer_font)
        footer_rect = QRectF(content_left, panel.bottom() - 30 * scale, content_width, 20 * scale)
        self.draw_text(painter, footer_rect, Qt.AlignCenter, "PRESS ANY KEY TO EXIT")

    def draw_metrics(self, painter, panel, scale, content_left, content_width):
        """Draw the live hardware section: load history, bars, and rates."""
        data = self.metrics_data
        painter.setFont(self.progress_font)
        header = "SYSTEM LOAD"
        if data["gpu_name"]:
            header = painter.fontMetrics().elidedText(f"{header} · {data['gpu_name']}", Qt.ElideRight, int(content_width))
        self.draw_text(painter, QRectF(content_left, panel.top() + 168 * scale, content_width, 20 * scale), Qt.AlignVCenter | Qt.AlignLeft, header)

        spark_rect = QRectF(content_left, panel.top() + 192 * scale, content_width, 28 * scale)
        self.draw_sparkline(painter, spark_rect, scale)

        cpu_text = f"{data['cpu_percent']:5.1f}%"
        self.draw_metric_row(painter, content_left, panel.top() + 226 * scale, content_width, scale, "CPU", data["cpu_percent"] / 100.0, cpu_text)
        if data["gpu_percent"] is not None:
            gpu_fraction = data["gpu_percent"] / 100.0
            gpu_text = f"{data['gpu_percent']:5.1f}%"
        else:
            gpu_fraction, gpu_text = 0.0, "  N/A"
        self.draw_metric_row(painter, content_left, panel.top() + 250 * scale, content_width, scale, "GPU", gpu_fraction, gpu_text)
        self.draw_metric_row(painter, content_left, panel.top() + 274 * scale, content_width, scale, "MEM", data["mem_percent"] / 100.0, f"{data['mem_percent']:5.1f}%")

        parts = [f"NET UP {format_rate(data['net_up'])}  DN {format_rate(data['net_down'])}"]
        if data["zone_temp"] is not None:
            parts.append(f"ZONE {data['zone_temp']:02.0f}C")
        if data["gpu_temp"] is not None:
            parts.append(f"GPU {data['gpu_temp']:02.0f}C")
        info_text = " · ".join(parts)
        hot = (data["zone_temp"] or 0) >= 80 or (data["gpu_temp"] or 0) >= 80
        painter.setPen(self.amber if hot else self.white)
        self.draw_text(painter, QRectF(content_left, panel.top() + 298 * scale, content_width, 20 * scale), Qt.AlignVCenter | Qt.AlignLeft, info_text)
        painter.setPen(self.green)

    def draw_metric_row(self, painter, left, top, width, scale, label, fraction, text):
        """Draw one label + bar + percent row for a hardware metric."""
        painter.setFont(self.progress_font)
        label_width = 52 * scale
        text_width = 118 * scale
        painter.setPen(self.white)
        self.draw_text(painter, QRectF(left, top, label_width, 20 * scale), Qt.AlignVCenter | Qt.AlignLeft, label)
        bar_rect = QRectF(left + label_width + 6 * scale, top + 3 * scale,
                          max(30 * scale, width - label_width - text_width - 12 * scale), 14 * scale)
        painter.setPen(QPen(self.progress_border, 1))
        painter.setBrush(self.progress_background)
        painter.drawRect(bar_rect)
        painter.setPen(Qt.NoPen)
        ...
        painter.drawRect(QRectF(bar_rect.left() + 1, bar_rect.top() + 1,
                                max(0, (bar_rect.width() - 2) * max(0.0, min(1.0, fraction))), bar_rect.height() - 2))
        painter.setPen(self.green)
        self.draw_text(painter, QRectF(bar_rect.right() + 6 * scale, top, text_width, 20 * scale), Qt.AlignVCenter | Qt.AlignRight, text)

    def draw_sparkline(self, painter, rect, scale):
        """Draw CPU (green) and GPU (blue) load history, anchored to the left."""
        painter.setPen(QPen(self.progress_border, 1))
        painter.setBrush(self.progress_background)
        ...
        if inner.width() <= 0 or inner.height() <= 0:
            return
        step = inner.width() / (HISTORY_LENGTH - 1)
        pen_width = max(1, round(1.5 * scale))
        for history, color in ((self.gpu_history, self.blue), (self.cpu_history, self.green)):
            if len(history) < 2:
                continue
            points = [QPointF(inner.left() + index * step, inner.bottom() - inner.height() * value / 100.0)
